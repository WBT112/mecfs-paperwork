...
const formpackSpaPaths = ["/formpacks/", "/formpacks/doctor-letter"];
const formpackPaths = [
...
const assertHeaderIncludes = (headerValue, expected, label) => {
  ...
  if (missing.length > 0) {
    throw new Error(
      `${label} header missing tokens: ${missing.join(", ")} (got: ${headerValue}).`,
    );
  }
};
...
const assertNoHsts = (headers, label) => {
  if (headers.has("strict-transport-security")) {
    throw new Error(
      `Strict-Transport-Security should not be set on HTTP responses (${label}).`,
    );
  }
};
...
const assertTextContains = (text, expected, label) => {
...
const run = async () => {
  const { response: indexResponse, text: indexHtml } = await fetchText(
    `${baseUrl}/`,
  );
  ...
  assertHeaderIncludes(
    deepLinkCache,
    ["no-cache", "must-revalidate"],
    "deep link Cache-Control",
  );
  assertNoHsts(deepLinkHeaders, "deep link");

  for (const formpackSpaPath of formpackSpaPaths) {
    const { response, text } = await fetchText(`${baseUrl}${formpackSpaPath}`);
    if (response.headers.get("content-type")?.includes("text/html") !== true) {
      throw new Error(
        `Expected SPA shell HTML response for ${formpackSpaPath}.`,
      );
    }
    assertTextContains(text, '<div id="root"></div>', formpackSpaPath);
  }

  for (const formpackPath of formpackPaths) {
    const formpackHeaders = await fetchHeaders(
      formpackPath,
      `formpack ${formpackPath}`,
    );
    const formpackCache = formpackHeaders.get("cache-control");
    assertHeaderIncludes(
      formpackCache,
      ["no-cache", "must-revalidate"],
      `formpack Cache-Control (${formpackPath})`,
    );
