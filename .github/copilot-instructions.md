...
- **Repo size:** Medium-sized mono-repo with one main app (`app/`) and docs/tools.
...
- **Install:**
  ...
  - After `git clean -xfd`, re-run `npm ci` to restore `node_modules` and rerun `npm run dev` or `npm run build` to regenerate build artifacts.
...
- This removes generated directories such as `app/dist/`, `app/playwright-report/`, and `app/test-results/`.
...
- `predev` validates that root `formpacks/` does not exist and syncs `.github/FUNDING.yml` → `app/src/lib/funding.generated.ts`.
...
- `app/` **main SPA**
  ...
  - `scripts/` automation: `assert-no-root-formpacks.mjs`, `sync-funding.mjs`, `validate-formpacks.mjs`, `new-formpack.mjs`, `run-e2e-soft.mjs`.
  - **Generated files:** `app/src/lib/funding.generated.ts` (do not edit; generated by predev/prebuild).
- `app/public/formpacks/` content packs (schemas, UI schemas, i18n, templates). See `docs/formpacks.md`.
...
`CODE_OF_CONDUCT.md`, `SECURITY.md`, `Dockerfile`, `Caddyfile`,
`Caddyfile.local`, `compose.yaml`, `compose.prod.yaml`, `compose.local-proxy.yaml`,
`app/`, `docs/`, `tools/`, `nginx/`, `package.json`, `package-lock.json`,
...
- Formpacks live in `app/public/formpacks/<id>/...` with schema/i18n/templates.
...
Formpacks are self-contained bundles in `app/public/formpacks/<id>/`:
...
- **Issue:** `app/node_modules` and `app/dist` removed.
